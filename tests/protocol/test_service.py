...
async def test_controller_delayed_shutdown_signal_handlers(controller_service) -> None:
    ...
    with patch('asyncio.sleep', new_callable=AsyncMock), \
         ...
         patch('builtins.hasattr', lambda obj, name: True if name == 'kill' and obj == os else original_hasattr(obj, name)):
        await controller_service._delayed_shutdown()
        mock_kill.assert_called_once_with(12345, signal.SIGTERM)

    with patch('asyncio.sleep', new_callable=AsyncMock), \
         patch('builtins.hasattr', lambda obj, name: False if name == 'kill' and obj == os else original_hasattr(obj, name)) as mock_hasattr_check, \
