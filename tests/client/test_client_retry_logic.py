...
@pytest.fixture
async def client_instance_local(mocker: Any) -> AsyncGenerator[RPCPluginClient]:
    ...
    mock_process_obj.returncode = (
        None  # Ensure returncode is part of the spec if accessed
    )
...
async def test_connect_handshake_retry_success_after_failures(
    client_instance_local: RPCPluginClient, mocker: Any
) -> None:
    ...
    async def side_effect_perform_handshake_with_failures() -> None:
...
@pytest.mark.asyncio
async def test_connect_handshake_retry_process_exits(
    client_instance_local: RPCPluginClient, mocker: Any
) -> None:
    ...
    if not isinstance(
        original_poll, MagicMock
    ):  # Ensure it's a mock we can control side_effect for
        client_instance._process.poll = MagicMock()

    poll_call_count = 0

    def mock_poll_side_effect(*args: Any, **kwargs: Any) -> int | None:
...
@pytest.mark.asyncio
async def test_connect_handshake_total_timeout_immediately(
    client_instance_local: RPCPluginClient, mocker: Any
) -> None:
    client_instance = client_instance_local
    mock_config_get = mocker.patch("pyvider.rpcplugin.config.rpcplugin_config.get")
    ...
    monotonic_values_sequence = [
        ...
        0.01,  # First check in loop (for while condition), time() - overall_start_time = 0.01 # noqa: E501
    ...
    def mock_monotonic_side_effect_func() -> float:
        ...
        try:
            ...
            # client_instance.logger.debug(f"Mock time.monotonic returning: {val}") # Optional: for debugging test # noqa: E501
            return val
        except StopIteration:
            # client_instance.logger.debug(f"Mock time.monotonic returning final value: {final_monotonic_value_after_timeout}") # noqa: E501
            return final_monotonic_value_after_timeout

    mocker.patch(
        "pyvider.rpcplugin.client.base.time.monotonic",
        side_effect=mock_monotonic_side_effect_func,
    )
    mocker.patch(
        "pyvider.rpcplugin.client.base.asyncio.sleep", new_callable=AsyncMock
    ...
    if not client_instance._process:
        client_instance._process = MagicMock(spec=subprocess.Popen)  # type: ignore[attr-defined] # noqa: E501
    ...
    client_instance.logger.error.assert_any_call(
        "Client connection/handshake retry sequence timed out after 0.0s. Last error: N/A"  # noqa: E501
    )
    assert client_instance._handshake_failed_event.is_set()


@pytest.mark.asyncio
async def test_connect_handshake_retry_disabled_failure(
    client_instance_local: RPCPluginClient, mocker: Any
) -> None:
    client_instance = client_instance_local
    mock_config_get = mocker.patch("pyvider.rpcplugin.config.rpcplugin_config.get")
    ...
    with pytest.raises(
        HandshakeError, match="Simulated handshake failure, retries disabled"
    ):
        await client_instance._connect_and_handshake_with_retry()

    assert client_instance._handshake_failed_event.is_set()
    client_instance.logger.error.assert_any_call(
        "Failed to connect and handshake with plugin (retries disabled): "
        "[HandshakeError] Simulated handshake failure, retries disabled",
        exc_info=True,
    )
...
@pytest.mark.asyncio
async def test_connect_handshake_retry_transport_close_fails(
    client_instance_local: RPCPluginClient, mocker: Any
) -> None:
    client_instance = client_instance_local
    mock_config_get = mocker.patch("pyvider.rpcplugin.config.rpcplugin_config.get")
    ...
    mock_transport_instance.close = AsyncMock(
        side_effect=Exception("Transport close failed")
    )

    # Ensure _perform_handshake sets up a transport that will then fail to close
    async def perform_handshake_sets_transport_then_fails() -> None:
        client_instance._transport = (
            mock_transport_instance  # Assign the mock transport
        )
    ...
    with pytest.raises(
        HandshakeError, match="Simulated first failure"
    ):  # Expect the final error
        await client_instance._connect_and_handshake_with_retry()
...
@pytest.mark.asyncio
async def test_connect_handshake_total_timeout_exceeded(
    client_instance_local: RPCPluginClient, mocker: Any
) -> None:
    ...
    simulated_error = HandshakeError(
        "Simulated persistent handshake failure for timeout test"
    )
    mock_perform_handshake = mocker.patch(
        "pyvider.rpcplugin.client.base.RPCPluginClient._perform_handshake",
        new_callable=AsyncMock,
    ...
    mock_logger_error = mocker.spy(client_instance.logger, "error")
    mock_logger_warning = mocker.spy(client_instance.logger, "warning")  # noqa: F841
    ...
    # 3rd call: inside loop, before sleep (attempt 2) - this should exceed total_timeout_s_config # noqa: E501
    monotonic_values_sequence = [
        0.0,  # Initial overall_start_time
        0.01,  # First check in loop (attempt 1)
        # Value for the time check that leads to timeout:
        total_timeout_s_config
        + 0.01,  # Ensures time.monotonic() - overall_start_time > total_timeout_s
    ]
    monotonic_iterator = iter(monotonic_values_sequence)
    ...
    def mock_monotonic_side_effect_func() -> float:
        try:
            ...
            client_instance.logger.debug(
                f"Mock time.monotonic returning final value: {final_monotonic_value_after_timeout}"  # noqa: E501
            )
            return final_monotonic_value_after_timeout

    mock_time_monotonic.side_effect = mock_monotonic_side_effect_func

    # We need actual sleep to allow monotonic time to be checked correctly if not fully mocked # noqa: E501
    ...
    expected_exception_message = (  # noqa: F841
        "Retry sequence timed out."  # If last_exception is None
    )
    # If last_exception is set, it would be "Simulated persistent handshake failure for timeout test" # noqa: E501
    # The code prioritizes last_exception if it exists.

    with pytest.raises(
        HandshakeError, match="Simulated persistent handshake failure for timeout test"
    ):
        await client_instance._connect_and_handshake_with_retry()
    ...
    # Check logger calls
    # This path is taken when the current attempt has failed, and scheduling the *next* sleep # noqa: E501
    # would push the total time over the limit.
    mock_logger_error.assert_any_call(
        f"Next retry would exceed total timeout. Aborting. Last error: {simulated_error}"  # noqa: E501
    )
    assert client_instance._handshake_failed_event.is_set()
...
@pytest.mark.asyncio
async def test_connect_handshake_max_retries_reached(
    client_instance_local: RPCPluginClient, mocker: Any
) -> None:
    ...
    mock_logger_error.assert_any_call(
        f"Maximum retry attempts ({max_retries_config + 1}) reached for "
        f"connection/handshake. Last error: {simulated_error}"
    )
    assert client_instance._handshake_failed_event.is_set()
