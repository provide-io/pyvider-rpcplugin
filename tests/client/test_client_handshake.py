...
@pytest.mark.asyncio
async def test_read_raw_handshake_line_process_exits_with_stderr(client_instance, mocker):
    ...
    with pytest.raises(HandshakeError, match="Plugin process exited with code 1 before handshake."):
        await client_instance._read_raw_handshake_line_from_stdout()

@pytest.mark.asyncio
async def test_read_raw_handshake_line_process_stdout_becomes_none(client_instance, mocker):
    ...
    mocker.patch('pyvider.rpcplugin.client.base.asyncio.sleep', side_effect=sleep_side_effect) # Target where it's used
    ...
    mock_executor_future.side_effect = lambda: asyncio.Future() # Return a new future each time
...
@pytest.mark.asyncio
async def test_read_raw_handshake_line_outer_timeout_with_stderr(client_instance, mocker):
    ...
    mock_loop_instance = mocker.MagicMock()
    mock_loop_instance.time.side_effect = [i * 1.0 for i in range(12)] # Enough to cause timeout
    ...
    mock_logger_error.assert_any_call("🤝 Handshake timed out. Stderr output: stderr messages on timeout")
...
async def test_perform_handshake_transport_not_initialized(client_instance, mock_process, mocker):
    ...
    original_parse_handshake_response = client_instance._perform_handshake.__globals__['parse_handshake_response']
    ...
    with pytest.raises(HandshakeError, match="Transport not initialized before connect call."):
        await client_instance._perform_handshake()
...
async def test_read_raw_handshake_line_byte_by_byte_success(client_instance, mocker):
    ...
    read_side_effect_list = [bytes([b]) for b in handshake_bytes] + [b''] # Store the list
    ...
    def custom_run_in_executor(loop, func_to_run):
        ...
        if executor_call_count == 1: # First call to executor is for readline
             asyncio.create_task(set_future_result(f, mock_stdout.readline.return_value)) # b""
            ...
            if read_call_idx < len(read_side_effect_list): # Use stored list here
                byte_to_return = read_side_effect_list[read_call_idx] # Access original list
    ...
    mocker.patch('pyvider.rpcplugin.client.base.asyncio.sleep') # Still mock sleep to prevent real delays
...
@pytest.mark.asyncio
async def test_read_raw_handshake_line_byte_by_byte_stdout_none(client_instance, mocker):
    ...
    mocker.patch('asyncio.get_event_loop').run_in_executor.side_effect = lambda _, func: func()
    mocker.patch('asyncio.wait_for', side_effect=lambda coro, timeout: coro)
    mocker.patch('pyvider.rpcplugin.client.base.asyncio.sleep') # Mock sleep to avoid delays
...
@pytest.mark.asyncio
async def test_read_raw_handshake_line_byte_by_byte_read_timeout(client_instance, mocker):
    ...
    async def set_future_result(fut, result_val, delay=0):
        ...
        if not fut.done(): fut.set_result(result_val)
    ...
    def run_in_executor_side_effect_for_timeout(loop, func):
        ...
        if 'readline' in qualname: # Mock readline part
            asyncio.create_task(set_future_result(fut, b"")) # readline returns empty to trigger byte-by-byte
    ...
    time_values = [i * 0.1 for i in range(105)]
    mock_loop_instance.time.side_effect = time_values
    mock_loop_instance.run_in_executor.side_effect = run_in_executor_side_effect_for_timeout

    mocker.patch('pyvider.rpcplugin.client.base.asyncio.get_event_loop', return_value=mock_loop_instance)
    mocker.patch('pyvider.rpcplugin.client.base.asyncio.wait_for', side_effect=wait_for_side_effect_for_read_timeout)
