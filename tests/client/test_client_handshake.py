...
async def test_perform_handshake_success(client_instance, mock_process):
    client_instance._process = mock_process
    with patch("pyvider.rpcplugin.client.base.RPCPluginClient._relay_stderr_background", new_callable=AsyncMock) as mock_relay,          patch("pyvider.rpcplugin.client.base.TCPSocketTransport") as mock_transport_class:
...
async def test_perform_handshake_with_unix_transport(client_instance, mock_process):
    client_instance._process = mock_process
    with patch("pyvider.rpcplugin.client.base.RPCPluginClient._relay_stderr_background", new_callable=AsyncMock) as mock_relay,          patch("pyvider.rpcplugin.client.base.UnixSocketTransport") as mock_transport_class:
        mock_transport_instance = AsyncMock()
        mock_transport_class.return_value = mock_transport_instance
...
async def test_perform_handshake_no_process(client_instance):
    ...
    with pytest.raises(HandshakeError, match="Plugin process or its stdout stream is not available for handshake."):
        await client_instance._perform_handshake()
...
async def test_perform_handshake_process_exit(client_instance, mock_process):
    ...
    with pytest.raises(HandshakeError, match=r"Plugin process exited prematurely \(code 1\) before handshake."):
        await client_instance._perform_handshake()
...
async def test_perform_handshake_parse_error(client_instance, mock_process):
    client_instance._process = mock_process
    mock_process.stdout.readline.return_value = b"1|1|tcp|127.0.0.1:8000|grpc|\n"
    with patch("pyvider.rpcplugin.client.base.RPCPluginClient._relay_stderr_background", new_callable=AsyncMock),          patch("pyvider.rpcplugin.client.base.parse_handshake_response", side_effect=ValueError("Simulated parse error")) as mock_parse,          pytest.raises(HandshakeError, match=r"Failed to process handshake response or establish transport connection: Simulated parse error"):
        await client_instance._perform_handshake()
...
@pytest.mark.asyncio
async def test_read_raw_handshake_line_process_exits_with_stderr(client_instance_for_retry_tests, mocker):
...
@pytest.mark.asyncio
async def test_read_raw_handshake_line_process_stdout_becomes_none(client_instance_for_retry_tests, mocker):
    client_instance = client_instance_for_retry_tests
    mock_process = client_instance._process
...
@pytest.mark.asyncio
async def test_read_raw_handshake_line_outer_timeout_with_stderr(client_instance_for_retry_tests, mocker):
    client_instance = client_instance_for_retry_tests
    mock_process = client_instance._process
    mock_process.poll.return_value = None
    ...
    expected_msg_regex = r"\[HandshakeError\] Timed out waiting for handshake line from plugin\. \(Hint: Ensure the plugin command '\['dummy-plugin-cmd'\]' starts correctly and outputs the handshake string to stdout within 10\.0 seconds\.\.\.\)"
...
@pytest.mark.asyncio
async def test_read_raw_handshake_line_outer_timeout_no_stderr(client_instance_for_retry_tests, mocker):
    client_instance = client_instance_for_retry_tests
    mock_process = client_instance._process
    mock_process.poll.return_value = None
    mock_process.stdout.readline.return_value = b""
...
@pytest.mark.asyncio
async def test_perform_handshake_transport_not_initialized(client_instance, mock_process, mocker):
    client_instance._process = mock_process
    ...
    mocker.patch("pyvider.rpcplugin.client.base.RPCPluginClient._relay_stderr_background", new_callable=AsyncMock)
    mocker.patch("pyvider.rpcplugin.client.base.parse_handshake_response", return_value=(1, 1, "tcp", "127.0.0.1:1234", "grpc", None))
    ...
    with pytest.raises(HandshakeError, match=r"Internal error: Transport was not initialized before attempting to connect."):
        await client_instance._perform_handshake()

@pytest.mark.asyncio
async def test_read_raw_handshake_line_byte_by_byte_success(client_instance_for_retry_tests, mocker):
    client_instance = client_instance_for_retry_tests
    mock_process = client_instance._process
    mock_process.poll.return_value = None
...
@pytest.mark.asyncio
async def test_read_raw_handshake_line_byte_by_byte_stdout_none(client_instance_for_retry_tests, mocker):
    client_instance = client_instance_for_retry_tests
    mock_process = client_instance._process
    mock_process.poll.return_value = None
    mock_process.stdout.readline.return_value = b""
...
async def test_read_raw_handshake_line_byte_by_byte_read_timeout(client_instance_for_retry_tests, mocker):
    ...
    mocker.patch("pyvider.rpcplugin.client.base.asyncio.wait_for", side_effect=custom_wait_for)
    ...
    mocker.patch("pyvider.rpcplugin.client.base.asyncio.get_event_loop", return_value=mock_loop_instance)
    mocker.patch("pyvider.rpcplugin.client.base.asyncio.sleep")
    with pytest.raises(HandshakeError, match=r"Timed out waiting for handshake line from plugin."): # General timeout
        await client_instance._read_raw_handshake_line_from_stdout()


@pytest.mark.asyncio
async def test_read_raw_handshake_line_process_exits_no_stderr(client_instance_for_retry_tests, mocker):
    client_instance = client_instance_for_retry_tests
    mock_process = client_instance._process
    mock_process.poll.return_value = 1
    mock_process.returncode = 1
...
@pytest.mark.asyncio
async def test_connect_handshake_retry_success_first_attempt(client_instance_for_retry_tests, mocker):
    client_instance = client_instance_for_retry_tests
    mock_config_get = mocker.patch("pyvider.rpcplugin.config.rpcplugin_config.get")
    config_values = {
        "PLUGIN_CLIENT_RETRY_ENABLED": "true", "PLUGIN_CLIENT_MAX_RETRIES": 3,
        "PLUGIN_CLIENT_INITIAL_BACKOFF_MS": 10, "PLUGIN_CLIENT_MAX_BACKOFF_MS": 100,
        "PLUGIN_CLIENT_RETRY_JITTER_MS": 5, "PLUGIN_CLIENT_RETRY_TOTAL_TIMEOUT_S": 5,
    }
    mock_config_get.side_effect = lambda key, default=None: config_values.get(key, default)

    mock_perform_handshake = mocker.patch("pyvider.rpcplugin.client.base.RPCPluginClient._perform_handshake", new_callable=AsyncMock)
    mock_create_grpc_channel = mocker.patch("pyvider.rpcplugin.client.base.RPCPluginClient._create_grpc_channel", new_callable=AsyncMock)
    ...
    logger_mock.info.assert_any_call("Handshake attempt 1 successful. Endpoint: mock_address, Transport: mock_transport")
    logger_mock.info.assert_any_call("Successfully connected to gRPC endpoint on attempt 1: mock_target_endpoint")
    logger_mock.info.assert_any_call("Client connection and handshake successful on attempt 1.")
    assert any("Starting connection/handshake sequence with retries enabled" in call.args[0] for call in logger_mock.info.call_args_list)
