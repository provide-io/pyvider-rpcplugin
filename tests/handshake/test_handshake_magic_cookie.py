...
def test_validate_magic_cookie_invalid(monkeypatch):
    ...
    monkeypatch.setitem(rpcplugin_config.config, "PLUGIN_MAGIC_COOKIE_KEY", "PLUGIN_MAGIC_COOKIE")
    monkeypatch.setitem(rpcplugin_config.config, "PLUGIN_MAGIC_COOKIE_VALUE", "hello")
    monkeypatch.setitem(rpcplugin_config.config, "PLUGIN_MAGIC_COOKIE", "invalid_cookie")
...
@pytest.mark.parametrize(
    ...
    [
        ("PLUGIN_MAGIC_COOKIE", "invalid_cookie", "cookie_provided does not match required cookie_value"),
        (None, None, ""),
        ("PLUGIN_MAGIC_COOKIE", None, "cookie not provided"),
        (None, "hello", "cookie_key not found"),
    ],
)
def test_validate_magic_cookie_failures(monkeypatch, magic_cookie_key, magic_cookie_value, expected_error):
    monkeypatch.setitem(rpcplugin_config.config, "PLUGIN_MAGIC_COOKIE_KEY", magic_cookie_key)
    monkeypatch.setitem(rpcplugin_config.config, "PLUGIN_MAGIC_COOKIE_VALUE", "hello")  # default
    monkeypatch.setitem(rpcplugin_config.config, "PLUGIN_MAGIC_COOKIE", magic_cookie_value)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("PLUGIN_MAGIC_COOKIE_KEY", "PLUGIN_MAGIC_COOKIE_VALUE", "PLUGIN_MAGIC_COOKIE_VALUE", False, None),
        # Missing environment variables
        (None, None, None, True, "cookie_key not found"),
        # Invalid cookie
        ("PLUGIN_MAGIC_COOKIE_KEY", "some_expected", "different_cookie", True, "cookie_provided does not match required cookie_value"),
    ],
)
def test_validate_magic_cookie(monkeypatch, set_key, set_value, set_cookie, expect_error, error_regex):
    """
