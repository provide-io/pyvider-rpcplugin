import os
from pyvider.rpcplugin.config import RPCPluginConfig, rpcplugin_config, configure, CONFIG_SCHEMA
...
def clear_all_known_plugin_env_vars():
    ...
    if cleared_keys:
        logger.debug(f"Cleared known PLUGIN_ environment variables: {', '.join(cleared_keys)}")
...
logger.info(f"configure() correctly overrode env var: {env_var_key}={updated_config_val}")
...
logger.info(f"Environment variable correctly unchanged: {env_var_key}={os.environ[env_var_key]}")
...
logger.debug(f"Set {key_protocol_versions}={os.environ[key_protocol_versions]} in environment.")
config = reinit_config()
...
logger.info(f"{key_protocol_versions} type conversion to list_int: OK ({actual_list_int})")
