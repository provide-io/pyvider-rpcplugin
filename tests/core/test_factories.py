...
def test_plugin_server_unix_transport_default_path(mock_unix_transport_cls, mock_rpc_plugin_server_cls):
    ...
    server = plugin_server(protocol=mock_protocol_inst, handler=mock_handler_inst, transport="unix")
...
def test_plugin_server_tcp_transport_default_host_port(mock_tcp_transport_cls, mock_rpc_plugin_server_cls):
    ...
    server = plugin_server(protocol=mock_protocol_inst, handler=mock_handler_inst, transport="tcp")
...
def test_plugin_server_invalid_transport():
    ...
    with pytest.raises(ValueError, match="Unsupported transport type: bogus"):  # Changed to ValueError
        plugin_server(protocol=mock_protocol_inst, handler=mock_handler_inst, transport="bogus")


@patch("pyvider.rpcplugin.factories.RPCPluginServer")
@patch("pyvider.rpcplugin.factories.TCPSocketTransport")
def test_plugin_server_tcp_transport_with_port(mock_tcp_transport_cls, mock_rpc_plugin_server_cls):
