...
def test_rate_limiter_init_invalid_capacity() -> None:
...
def test_rate_limiter_init_invalid_refill_rate() -> None:
...
@pytest.mark.asyncio
async def test_rate_limiter_denies_when_empty(mocker: Any) -> None:  # Removed caplog
    # Patch time.monotonic to control time precisely
    mock_time: MagicMock = mocker.patch("time.monotonic")
    mock_logger_warning: MagicMock = mocker.patch(
        "pyvider.rpcplugin.rate_limiter.logger.warning"
    )  # Patch logger
    ...
    for call_args_tuple in mock_logger_warning.call_args_list:
        ...
        if args and "Request denied. No tokens available." in args[0]:
            ...
            break
    assert found_log, (
        f"Expected 'Request denied' log not found. Logs: {mock_logger_warning.call_args_list}"  # noqa: E501
    )


@pytest.mark.asyncio
async def test_get_current_tokens(mocker: Any) -> None:
    mock_time: MagicMock = mocker.patch("time.monotonic")
    mock_time.return_value = 1000.0
    ...
    # Current tokens = 5 (initial) - 1 (consumed) = 4. Refill is not counted by get_current_tokens. # noqa: E501
    # The internal _tokens value right after consumption is 4.0
    assert await limiter.get_current_tokens() == 4.0


@pytest.mark.asyncio
async def test_rate_limiter_refills_over_time(mocker: Any) -> None:
    mock_time: MagicMock = mocker.patch("time.monotonic")
...
@pytest.mark.asyncio
async def test_refill_does_not_exceed_capacity(mocker: Any) -> None:
    mock_time: MagicMock = mocker.patch("time.monotonic")
    ...
    # To test _refill_tokens directly, we need to acquire the lock or make it public for test # noqa: E501
    # For now, let's test through is_allowed or get_current_tokens if it refills.
    # The current get_current_tokens does NOT refill.
    # So, to check refill, we need to call is_allowed or simulate its internal lock + refill. # noqa: E501
    ...
    # Let's verify _refill_tokens more directly by accessing internal state (not ideal but for 100%) # noqa: E501
    mock_time.return_value = 1000.0
...
@pytest.mark.asyncio
async def test_refill_no_time_passed(mocker: Any) -> None:
    mock_time: MagicMock = mocker.patch(  # noqa: F841
        "time.monotonic", return_value=1000.0
    )
    limiter = TokenBucketRateLimiter(capacity=5, refill_rate=1)

    initial_tokens = await limiter.get_current_tokens()  # noqa: F841

    # Call _refill_tokens (indirectly, or directly if made testable) when no time has passed # noqa: E501
