...
import pytest
import os
from pyvider.rpcplugin.config import RPCPluginConfig, CONFIG_SCHEMA, validate_config_value, ConfigError
...
def auto_clean_rpc_config_env():
    ...
    _ = RPCPluginConfig.instance() # Re-init with restored env for subsequent modules/tests
...
def test_default_value_fallbacks():
    ...
    assert actual_cookie == default_cookie, f"Default magic_cookie_value: expected '{default_cookie}', got '{actual_cookie}'"
    ...
    assert actual_auto_mtls == default_auto_mtls_bool, f"Default auto_mtls_enabled: expected {default_auto_mtls_bool}, got {actual_auto_mtls}"
    ...
    assert actual_handshake_timeout == default_handshake_timeout, f"Default handshake_timeout: expected {default_handshake_timeout}, got {actual_handshake_timeout}"
    ...
    assert actual_log_level == default_log_level, f"Default PLUGIN_LOG_LEVEL: expected '{default_log_level}', got '{actual_log_level}'"
...
def test_invalid_handshake_timeout_type(monkeypatch):
    ...
    with pytest.raises(ConfigError) as excinfo: # Changed from ValueError to ConfigError for more specificity if desired
        force_reinit_config_for_test() # This will trigger validation during RPCPluginConfig init

    assert "Invalid value format for configuration key 'PLUGIN_HANDSHAKE_TIMEOUT'" in str(excinfo.value)
    logger.info(f"Successfully caught ConfigError for invalid float type. Error: {str(excinfo.value)}")
...
def test_invalid_log_level_enum(monkeypatch):
    ...
    with pytest.raises(ConfigError) as excinfo: # Changed from ValueError to ConfigError
        force_reinit_config_for_test()

    assert "Invalid value 'NOT_A_VALID_LOG_LEVEL' provided for configuration key 'PLUGIN_LOG_LEVEL'" in str(excinfo.value)
    logger.info(f"Successfully caught ConfigError for invalid log level enum. Error: {str(excinfo.value)}")

def test_missing_required_value_direct_call():
    key_to_test_required = "PLUGIN_MAGIC_COOKIE_VALUE"
    logger.info(f"Testing validate_config_value directly for missing required value: {key_to_test_required}")
    ...
    assert f"Missing required configuration key: '{key_to_test_required}'" in str(excinfo.value)
    logger.info(f"Successfully caught ConfigError for missing required value via direct call. Error: {str(excinfo.value)}")
