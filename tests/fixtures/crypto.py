...
def client_cert(dev_root_ca: Certificate) -> Certificate:
    ...
    if not cert_to_use or not key_to_use:
        logger.info("PLUGIN_CLIENT_CERT or PLUGIN_CLIENT_KEY not found/PEM-like in env, generating default client cert for tests.")
...
def server_cert(dev_root_ca: Certificate) -> Certificate:
    ...
    if not cert_to_use or not key_to_use:
        logger.info("PLUGIN_SERVER_CERT or PLUGIN_SERVER_KEY not found/PEM-like in env, generating default server cert for tests.")
...
def temporary_key_file(tmp_path, client_cert: Certificate) -> str: # Added type hint
    ...
    if client_cert.key: # Ensure key is not None before writing
        ...
        logger.warning("Client certificate fixture had no key, temporary key file is empty.")
