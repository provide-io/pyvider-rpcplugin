...
async def server_instance(rpc_plugin_server_manager, mock_server_protocol, mock_server_handler, mock_server_config_dict_fixture):
    """
...
async def rpc_plugin_server_manager(managed_unix_socket_path, unused_tcp_port, mock_server_protocol, mock_server_handler):
    """
    ...
    async def _create_server(
        ...
        from pyvider.rpcplugin.config import rpcplugin_config # Import here to use fresh instance
        ...
        protocol_to_use = protocol or mock_server_protocol # Use provided fixture as default
        handler_to_use = handler or mock_server_handler   # Use provided fixture as default
        ...
        if custom_transport:
            ...
            logger.debug(f"RPCPluginServerManager: Using Unix socket path: {socket_path}")
        ...
        if auto_start:
            ...
            try:
                ...
                if server.transport and server.transport.endpoint:
                    ...
                    logger.debug(f"RPCPluginServerManager: Server ready at {endpoint}. Serve task: {serve_task}")
                else:
                    logger.error("RPCPluginServerManager: Server reported ready, but transport endpoint is missing.")
                    serve_task.cancel() # Cancel if endpoint is missing
                    await asyncio.gather(serve_task, return_exceptions=True)
                    raise RuntimeError("Server transport endpoint not available after start.")
            except Exception as e:
                logger.error(f"RPCPluginServerManager: Error starting server or waiting for readiness: {e}")
                serve_task.cancel()
                await asyncio.gather(serve_task, return_exceptions=True)
                raise
    ...
    # Cleanup
    logger.debug(f"RPCPluginServerManager: Cleaning up {len(servers_to_cleanup)} server(s).")
    for server_instance_to_clean in servers_to_cleanup:
        try:
            await server_instance_to_clean.stop()
            logger.debug(f"RPCPluginServerManager: Stopped server: {server_instance_to_clean}")
        except Exception as e:
            logger.error(f"RPCPluginServerManager: Error stopping server {server_instance_to_clean}: {e}")
