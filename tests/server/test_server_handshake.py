...
async def test_server_handshake_invalid_cookie(
    ...
    monkeypatch.setitem(
        rpcplugin_config.config, "PLUGIN_MAGIC_COOKIE_VALUE", "valid_cookie"
    )
    ...
    monkeypatch.setitem(
        rpcplugin_config.config, "PLUGIN_SERVER_TRANSPORTS", ["tcp", "unix"]
    )
...
async def test_negotiate_handshake_via_negotiation(
    ...
    monkeypatch.setitem(
        rpcplugin_config.config, "PLUGIN_SERVER_TRANSPORTS", [transport_type]
    )
