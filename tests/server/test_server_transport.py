...
async def test_setup_server_unix_success_insecure(
    ...
    def mock_config_get_insecure(key, default=None):
        ...
        if key == "PLUGIN_SERVER_KEY":
            return None
        return mock_server_config.config.get(
            key, default
        )  # Use .config for direct dict access
...
async def test_setup_server_exception_1(
    ...
    with pytest.raises(TransportError, match=r"Socket .* is already running"):
...
async def test_setup_server_exception_2(
    ...
    from unittest import (
        mock,
    )  # Already imported at top, but kept for clarity if this test is isolated
    ...
    elif hasattr(transport, "_running") and transport._running:
        await transport.close()
...
async def test_setup_server_unix_no_socket_linux_1(
    ...
    mock_cert_instance.cert = (
        "dummy_cert_pem_content_for_test_setup_server_unix_no_socket_linux_1"
    )
    mock_cert_instance.key = (
        "dummy_key_pem_content_for_test_setup_server_unix_no_socket_linux_1"
    )
    mock_certificate_class.return_value = mock_cert_instance
...
async def test_setup_server_tcp_success(
    ...
    if (
        not transport.endpoint
        or not hasattr(transport, "_server")
        or not transport._server
    ):
        await transport.listen()
    ...
    assert (
        "127.0.0.1" in server_instance._transport.endpoint
        and not server_instance._transport.endpoint.startswith("unix:")
