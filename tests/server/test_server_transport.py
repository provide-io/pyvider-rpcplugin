...
async def test_setup_server_unix_success_secure(
    ...
    mocker.patch.object(
        server, "_generate_server_credentials", return_value="mock_secure_creds"
    )
    ...
    mocker.patch(
        "pyvider.rpcplugin.server.GRPCServer", return_value=mock_grpc_server_instance
    )
    ...
    try:
        ...
        mock_grpc_server_instance.add_secure_port.assert_called_once_with(
            f"unix:{sock_path}", "mock_secure_creds"
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            RuntimeError("Failed to bind to socket"),
            r"gRPC server failed to start: Failed to bind to socket",
        ),
        (
...
async def test_setup_server_add_port_failure(
    ...
    mocker.patch.object(
        server, "_generate_server_credentials", return_value="mock_creds"
    )

    with mock.patch.object(
        dummy_server, "add_secure_port", side_effect=raised_exception
    ):
