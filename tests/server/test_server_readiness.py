...
class DummyProtocol(
    RPCPluginProtocol[MagicMock, DummyHandler]
):  # Use MagicMock for ServerT
...
async def test_wait_for_server_ready_unix_path_not_exists(mocker):
    ...
    server = RPCPluginServer(
        protocol=mock_protocol, handler=mock_handler, transport=mock_unix_transport
    )
    ...
    with pytest.raises(
        TransportError, match=r"Unix socket file /tmp/test_unix.sock does not exist."
    ):
...
async def test_wait_for_server_ready_tcp_port_none(mocker):
    ...
    server = RPCPluginServer(
        protocol=mock_protocol, handler=mock_handler, transport=mock_tcp_transport
    )
    ...
    with pytest.raises(
        TransportError, match="TCP port not available for readiness check."
    ):
        await server.wait_for_server_ready(timeout=0.1)
...
async def test_wait_for_server_ready_tcp_connect_fails(mocker):
    ...
    with pytest.raises(
        TransportError, match="Server readiness check failed: Connection failed"
    ):
        await server.wait_for_server_ready(timeout=0.1)
