...
@pytest.mark.asyncio
async def test_check_socket_in_use_stat_oserror(
...
@pytest.mark.asyncio
async def test_check_socket_in_use_sock_close_exception(
    mocker, managed_unix_socket_path, caplog
):
    transport = UnixSocketTransport(path=managed_unix_socket_path)
    ...
    mock_socket_instance.close = MagicMock(
        side_effect=Exception("Failed to close test socket")
    )
...
@pytest.mark.asyncio
async def test_check_socket_in_use_connect_other_oserror(
    mocker, managed_unix_socket_path, caplog
):
    transport = UnixSocketTransport(path=managed_unix_socket_path)
    mock_socket_instance = MagicMock(spec=socket.socket)
    ...
    assert (
        "Permission denied" in args[0]
    )  # This is part of the exception string in the log
    assert "Assuming available" in args[0]
...
async def test_check_socket_in_use_connect_specific_errors(
    mocker, managed_unix_socket_path, error_to_raise
):
    transport = UnixSocketTransport(path=managed_unix_socket_path)
    mock_socket_instance = MagicMock(spec=socket.socket)
...
def test_normalize_unix_path_double_slash():
    from pyvider.rpcplugin.transport.unix import (
        normalize_unix_path,
