...
async def test_unix_socket_connect_invalid_endpoint() -> None:
    ...
    transport = UnixSocketTransport(
        path="/tmp/this/better/be/an/invalid/endpoint-!@#%!#$!@$"  # nosec B108
    )
...
@pytest.mark.asyncio
async def test_unix_connect_retries_on_path_not_exists(
    ...
    for call_args_list_item in mock_logger_debug.call_args_list:
        ...
        if args and "Socket file not found, retrying" in args[0]:
            ...
            break
    assert found_log, (
        f"Expected retry log message not found. Actual logs: {mock_logger_debug.call_args_list}"
    )
...
async def test_unix_connect_stat_fails(mocker, managed_unix_socket_path):
    ...
    with pytest.raises(
        TransportError, match="Error checking socket status: stat failed"
    ):
