...
async def test_unix_connect_success(monkeypatch, managed_unix_socket_path) -> None:
    ...
    monkeypatch.setattr("os.path.exists", lambda path_arg: True if path_arg == sock_path else original_os_path_exists(path_arg))
    ...
    monkeypatch.setattr("os.stat", lambda path_arg, *args, **kwargs: mock_stat_obj if path_arg == sock_path else original_os_stat(path_arg, *args, **kwargs))
    ...
    monkeypatch.setattr(stat, "S_ISSOCK", lambda mode_arg: True if mode_arg == socket_st_mode else original_s_issock(mode_arg))
