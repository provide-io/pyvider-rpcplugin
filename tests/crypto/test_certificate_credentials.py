...
def mock_ssl_server_credentials(
    ...
    if require_client_auth and root_certificates is None:
        raise ValueError("root_certificates is required when require_client_auth is True")
    ...
    for private_key, certificate_chain in private_key_certificate_chain_pairs:
        if not isinstance(private_key, bytes) or not isinstance(certificate_chain, bytes):
