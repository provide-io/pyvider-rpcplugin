...
async def client_task(client_id, protocol_def, dummy_handshaker_path, server_connect_address_str):
    ...
    client = plugin_client(
        server_path=dummy_handshaker_path,
        protocol=protocol_def
    )
    stub = None
    try:
...
async def main():
    ...
    socket_path = "/tmp/bench_cc_echo.sock"
    server_address_uri = f"unix://{socket_path}" # Used by server and for dummy handshaker output

    dummy_handshaker_script_path = str(benchmarks_dir / "dummy_echo_handshaker.sh")
    ...
    print(f"Dummy handshaker {dummy_handshaker_script_path} configured to output: {handshake_line_for_dummy}")
    ...
    server_task = asyncio.create_task(run_server(actual_protocol_def, handler, server_address_uri, ready_event))

    results_summary = {"successful": 0, "failed": 0, "error": "None"}
    try:
        print(f"Waiting for server for {server_address_uri} to be ready...")
        await asyncio.wait_for(ready_event.wait(), timeout=10.0)
        print("Server is ready. Starting concurrent clients...")

        num_concurrent_clients = 150
        ...
        tasks = [client_task(i, actual_protocol_def, dummy_handshaker_script_path, server_address_uri) for i in range(num_concurrent_clients)]
        ...
        for i, r_exc in enumerate(client_run_results):
            if isinstance(r_exc, bool) and r_exc:
                ...
                print(f"Client task {i} resulted in exception during gather: {type(r_exc).__name__} - {r_exc}")
        ...
        print(f"Concurrency Test: {successful_calls}/{num_concurrent_clients} clients made successful calls.")

    except asyncio.TimeoutError:
        print("Server did not become ready in time.")
        results_summary["error"] = "Server Timeout"
    except Exception as e:
        print(f"Error during main concurrency execution: {type(e).__name__} - {e}")
        import traceback
        traceback.print_exc()
        results_summary["error"] = str(e)
    finally:
        print("Concurrency benchmark: Main section finished. Stopping server...")
    ...
    print(f"Concurrent Clients: Successful Calls={results_summary['successful']}, Failed/Not True={results_summary['failed']}, Overall Error='{results_summary.get('error', 'None')}'")
    print("Concurrency benchmark script finished.")
