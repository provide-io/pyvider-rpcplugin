...
async def run_server_async(protocol, handler, address_url, ready_event, actual_socket_path_or_port):
    ...
    if transport_type == "unix":
        ...
        host_part = path_or_host_port.split(":")[0] if ":" in path_or_host_port else "localhost"
        kwargs["host"] = host_part
        kwargs["port"] = int(actual_socket_path_or_port)

    server = plugin_server(**kwargs)
    print(f"Server starting with args: {kwargs}") # This will print the handshake string to server's stdout
...
async def measure_connection_time(protocol, dummy_server_script_path, num_iterations):
    timings = []
    print(f"Measuring connection time for {num_iterations} iterations using {dummy_server_script_path}...")
...
async def main_test_address(protocol, handler, address_url, actual_socket_path_or_port, dummy_handshaker_content_generator):
    ...
    try:
        ...
        await asyncio.wait_for(server_ready_event.wait(), timeout=10.0) # Increased server ready timeout slightly
        print(f"Server for {address_url} is ready. Running client connection speed tests for {num_iterations} iterations...")
        connection_timings = await measure_connection_time(protocol, dummy_handshaker_script_path, num_iterations)
    ...
    if connection_timings:
        print(f"Connection Speed for {address_url}: Avg={statistics.mean(connection_timings):.2f}ms, Med={statistics.median(connection_timings):.2f}ms, Min={min(connection_timings):.2f}ms, Max={max(connection_timings):.2f}ms from {len(connection_timings)} samples")
    else:
...
async def main():
    ...
    results["unix"] = await main_test_address(protocol, handler, unix_address, unix_socket_path, get_unix_dummy_handshaker_content)

    tcp_port = 50052
    tcp_address = f"tcp://localhost:{tcp_port}"
    print("\n--- Testing TCP Socket Connection Speed ---")
    results["tcp"] = await main_test_address(protocol, handler, tcp_address, tcp_port, get_tcp_dummy_handshaker_content)

    print("\n--- Benchmark Results ---")
    if results["unix"]:
        print(f"Unix Connection Speed: Avg={statistics.mean(results['unix']):.2f}ms, Med={statistics.median(results['unix']):.2f}ms")
    else:
        print("Unix Connection Speed: No successful connections.")
    if results["tcp"]:
        print(f"TCP Connection Speed: Avg={statistics.mean(results['tcp']):.2f}ms, Med={statistics.median(results['tcp']):.2f}ms")
    else:
