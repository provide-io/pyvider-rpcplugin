...
class KVClient:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if self._client and self._client._process and self._client._process.stderr:
                ...
                logger.warning("📝 Server process or stderr not available for relay at client start.")
            ...
            logger.error(f"🚨 Failed to connect/start KV server: {type(e).__name__} - {e}")
    ...
    def _relay_stderr(self) -> None:
        ...
        if not (self._client and self._client._process and self._client._process.stderr):
            logger.warning("📝 stderr relay: client process or stderr not available at relay setup.")
            return
            ...
            while True:
                try:
                    ...
                    if stderr_pipe.closed:
                        ...
                        break
                    if not self._client or not self._client._process: # Client or process might be cleaned up
                        logger.debug("📝 stderr relay: client or process no longer exists.")
                        break
                    ...
                    logger.error(f"📝 Error in stderr relay thread: {type(e).__name__} - {e}")
                    break
    ...
    async def close(self) -> None:
        if hasattr(self, '_client') and self._client:
            ...
            try:
                ...
                logger.error(f"🔒 Error during RPCPluginClient.close(): {type(e).__name__} - {e}")
            ...
            logger.debug("🔒 Close called but _client was not initialized or already None.")
    ...
    async def put(self, key: str, value: bytes) -> None:
        ...
        try:
            logger.debug(f"🗣️ 📝 Client: Sending Put request - key='{key}', value_size={len(value)} bytes.")
            ...
            logger.error(f"🗣️ 📝 Client: Put operation failed for key='{key}'. Error: {type(e).__name__} - {e}")
            raise
    ...
    async def get(self, key: str) -> bytes | None:
        ...
        try:
            ...
            if e.code() == grpc.StatusCode.NOT_FOUND:
                logger.info(f"🗣️ 📚 Client: Key='{key}' not found on server (gRPC StatusCode.NOT_FOUND).")
                return None
            logger.error(f"🗣️ 📚 Client: Get operation for key='{key}' failed with gRPC error {e.code()}: {e.details()}")
            raise
        except Exception as e:
            logger.error(f"🗣️ 📚 Client: Get operation for key='{key}' failed. Error: {type(e).__name__} - {e}")
            raise
...
async def main() -> None:
    ...
    default_go_server_path = (script_dir.parent / "go-rpc" / "bin" / "kv-go-server").resolve()
    ...
    if args.server_path:
        logger.info(f"Using server path from --server-path argument: {args.server_path}")
        ...
        if temp_path.exists() and os.access(temp_path, os.X_OK):
            ...
            logger.error(f"🚨 Server path from --server-path ('{args.server_path}') does not exist or is not executable.")
            sys.exit(1)
    else:
        server_path_env = os.environ.get("PLUGIN_SERVER_PATH")
        if server_path_env:
            logger.info(f"Using server path from PLUGIN_SERVER_PATH environment variable: {server_path_env}")
            temp_path = Path(server_path_env)
            base_dirs_for_relative = [Path.cwd(), Path("/app") if Path("/app").exists() else None, script_dir]
            base_dirs_for_relative = [d for d in base_dirs_for_relative if d] # Filter out None

            search_paths = [temp_path] if temp_path.is_absolute() else [base / temp_path for base in base_dirs_for_relative]
            ...
            for sp in search_paths:
                ...
                if sp_resolved.exists():
                    if os.access(sp_resolved, os.X_OK):
                        ...
                        logger.info(f"Found server via PLUGIN_SERVER_PATH ('{server_path_env}' -> '{resolved_server_path_str}')")
                        found_env_path = True
                        break
                    else:
        ...
        if not resolved_server_path_str: # If env var not set or path not valid
            ...
            elif default_py_server_path.exists() and os.access(default_py_server_path, os.X_OK):
                resolved_server_path_str = str(default_py_server_path)
                logger.info(f"Using default Python server: {resolved_server_path_str}")
            else:
                logger.error(f"🚨 Default Go server ({default_go_server_path}) and Python server ({default_py_server_path}) not found or not executable.")
                sys.exit(1)
    ...
    try:
        ...
        if args.command == "put":
            ...
            logger.info(f"📝 Executing PUT: key='{args.key}', value (bytes)={put_value_bytes!r}")
        ...
        if client_started_successfully:
            logger.info("🔒 Closing client connection in `finally` block (client was started).")
            await client.close()
        elif hasattr(client, '_client') and client._client is not None:
             logger.info("🔒 Client start failed or did not complete, attempting cleanup of partially initialized client.")
             await client.close()
        else:
             logger.info("🏁 Operation completed (client was not started or already cleaned up).")
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(f"🚨 Critical unhandled error in script execution: {type(e).__name__} - {e}")
