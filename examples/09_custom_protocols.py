...
class CustomDataStreamProtocol(RPCPluginProtocol):
    ...
    async def get_grpc_descriptors(self) -> Tuple[Any, str]:
        ...
        mock_descriptor = {
            "service_name": self.service_name,
            "methods": ["StreamData", "BatchProcess", "GetMetrics"],
            "features": {
        ...
        class EnhancedHandler:
            ...
            async def _process_batch(self, batch: List[Any]) -> List[Any]:
                ...
                if self.protocol.compression:
                    ...
                    self.protocol.metrics["compression_ratio"] = (
                        compressed_size / original_size
                    )

                # Update metrics
                self.protocol.metrics["messages_processed"] += len(batch)
                self.protocol.metrics["bytes_transferred"] += sum(
                    len(str(item)) for item in batch
                )
    ...
    def _create_adaptive_handler(self, handler: Any) -> Any:
        ...
        class AdaptiveHandler:
            ...
            async def ProcessData(self, request, context):
                ...
                compression_ratio = self.protocol.compression_stats.get(
                    content_type, 0.8
                )
...
async def example_9_custom_streaming_protocol():
    ...
    streaming_protocol = CustomDataStreamProtocol(
        service_name="DataStreamService", compression=True, batch_size=50
    )
    ...
    server = plugin_server(
        protocol=streaming_protocol, handler=handler, transport="unix"
    )
...
async def example_9_adaptive_compression():
    ...
    # Create server
    server = plugin_server(
        protocol=compression_protocol, handler=handler, transport="unix"
    )
...
async def example_9_versioned_api():
    ...
    versioned_protocol = VersionedProtocol(
        service_name="VersionedAPIService", supported_versions=["v1", "v2", "v3"]
    )
    ...
    # Create server
    server = plugin_server(
        protocol=versioned_protocol, handler=handler, transport="unix"
    )
...
async def example_9_protocol_composition():
        ...
        def _wrap_with_middleware(self, handler):
            ...
            class MiddlewareWrappedHandler:
                ...
                async def ProcessWithMiddleware(self, request, context):
                    ...
                    # Core processing
                    if hasattr(self.base_handler, "process_data"):
                        result = await self.base_handler.process_data(
                    ...
                    return {
                        "result": result,
                        "middleware_applied": len(self.middleware_stack),
                    }
    ...
    class MetricsMiddleware:
        ...
        async def post_process(self, result, context):
            ...
            logger.debug(
                ...
                avg_processing_time_ms=(self.total_processing_time / self.request_count)
                * 1000,
            )
            return result
    ...
    class CachingMiddleware:
        ...
        async def post_process(self, result, context):
            if not hasattr(context, "cached_result"):
                # Store in cache
                request_key = (
                    "sample_key"  # In real implementation, would derive from request
                )
    ...
    # Create server
    server = plugin_server(
        protocol=composite_protocol, handler=handler, transport="unix"
    )
