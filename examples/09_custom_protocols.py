...
class CustomProtocol(RPCPluginProtocol):
    ...
    async def add_to_server(self, server: Any, handler: Any) -> None:
        ...
        logger.info(f"✅ {self.service_name} registered with {len(self.middleware_stack)} middleware")
...
class LoggingMiddleware:
    ...
    async def __getattr__(self, name):
        ...
        raise AttributeError(f"'{self.handler.__class__.__name__}' has no attribute '{name}'")
        ...
        if hasattr(self.handler, name):
            ...
            async def timed_method(*args, **kwargs):
                ...
                try:
                    ...
                    duration = time.time() - start_time
                    logger.error(f"⏱️  [TIMING] {name} failed after {duration:.3f}s: {e}")
                    raise
