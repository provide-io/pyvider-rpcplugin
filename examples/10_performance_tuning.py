...
class PerformanceBenchmarker:
    ...
    async def benchmark_rpc_performance(
        ...
        try:
            ...
            async def client_worker(
                ...
                channel = (
                    None  # Define channel here to ensure it's available in finally
                )
            ...
            transport_type = server_config.get(
                "transport", "unix"
            )  # Get transport type from server_config
            ...
            if latencies:
                ...
                avg_latency = p95_latency = p99_latency = min_latency = max_latency = (
                    0.0
                )
    ...
    async def compare_transport_performance(self) -> dict[str, PerformanceMetrics]:
        ...
        tcp_config = {"transport": "tcp", "host": "127.0.0.1", "port": 0}
        ...
        logger.info(
            ...
            unix_faster_by=f"{performance_ratio:.1f}x"
...
async def example_10_transport_optimization():
    ...
    comparison = {
        "throughput": {
            ...
            "winner": "unix"
            if unix_metrics.requests_per_second > tcp_metrics.requests_per_second
            else "tcp",
        },
        "latency": {
            ...
            "winner": "unix"
            if unix_metrics.p99_latency_ms < tcp_metrics.p99_latency_ms
            else "tcp",
        },
        "resources": {
            ...
            "cpu_winner": "unix"
            if unix_metrics.cpu_usage_percent < tcp_metrics.cpu_usage_percent
            else "tcp",
            "memory_winner": "unix"
            if unix_metrics.memory_usage_mb < tcp_metrics.memory_usage_mb
            else "tcp",
        },
    }
    ...
    if comparison["resources"]["unix_cpu"] < comparison["resources"]["tcp_cpu"]:
        recommendations.append("Unix sockets have lower CPU overhead")
...
async def example_10_concurrency_tuning():
    ...
    async def test_concurrency_level(
        concurrent_clients: int, requests_per_client: int
    ) -> PerformanceMetrics:
    ...
    class MemoryOptimizedHandler:
        ...
        async def ProcessRequest(self, request, context):
            ...
            if message in self.cache:
                ...
                return type(
                    "Reply", (), {"response": self.cache[message], "from_cache": True}
                )()
