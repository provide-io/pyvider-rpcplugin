# env.ps1 - pyvider-rpcplugin Development Environment Setup
#
...
if (-not (Test-Path "pyproject.toml")) {
    ...
    Write-Host "Please run this script from the pyvider-rpcplugin root directory"
    exit 1
}
...
$Profile = if ($env:PYVIDER_RPCPLUGIN_PROFILE) { $env:PYVIDER_RPCPLUGIN_PROFILE } else { "default" }
if ($Profile -eq "default") {
    $VenvDir = "workenv/pyvider-rpcplugin_${TFOS}_${TFARCH}"
} else {
...
$LogDir = Join-Path $env:TEMP "pyvider-rpcplugin_setup"
...
Write-Host "Installing pyvider-rpcplugin in editable mode..." -NoNewline
try {
...
Write-Host "`n$("pyvider-rpcplugin development environment activated" | Write-Host -ForegroundColor Green)"
Write-Host "Virtual environment: $VenvDir"
Write-Host "Profile: $Profile"
Write-Host "`nUseful commands:"
Write-Host "  pyvider-rpcplugin --help  # pyvider-rpcplugin CLI"
