...
*   When the **Host Application launches a Plugin Executable**:
    ...
    *   Example: If the host application (acting as a client to the plugin) is configured with `PLUGIN_MAGIC_COOKIE_KEY="AUTH_TOKEN"` and `PLUGIN_MAGIC_COOKIE_VALUE="secretXYZ"`, then the plugin executable it launches must be started with the environment variable `AUTH_TOKEN="secretXYZ"`.
    *   The `pyvider.rpcplugin.plugin_client()` factory function, when used to launch a plugin, automatically handles this: it reads `PLUGIN_MAGIC_COOKIE_KEY` and `PLUGIN_MAGIC_COOKIE_VALUE` from its own (the host application's) configuration and sets the correctly named environment variable with the correct value for the plugin subprocess it launches.

*   **Server-Side Validation** (performed by `validate_magic_cookie` in `handshake.py` which is called by `RPCPluginServer` running within the plugin executable):
    1.  The plugin server retrieves the expected secret value from its own `PLUGIN_MAGIC_COOKIE_VALUE` configuration (which should match what the host intended).
