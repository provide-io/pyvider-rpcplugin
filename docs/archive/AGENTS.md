...
1. **Transport Abstraction**: All network communication goes through the `RPCPluginTransport` interface
1. **Async-First**: Built for `asyncio` with comprehensive async/await support
1. **Security by Default**: mTLS with certificate management, handshake validation
1. **Type Safety**: Modern Python typing with `attrs` classes, no legacy `Dict`/`List` types
...
1. Secure handshake with magic cookie validation
1. Protocol version negotiation
1. Transport type negotiation (Unix vs TCP)
1. mTLS certificate exchange
1. Service registration and RPC communication
...
1. **Import errors**: Ensure PYTHONPATH includes `src/` (automatically configured in pytest via `pythonpath = ["src"]` in pyproject.toml)
1. **Type checking with generated protobuf files**: Already excluded in mypy/pyre configuration
1. **Async event loop conflicts**: Use `pytest-asyncio` fixtures and `asyncio_mode = "auto"`
1. **Foundation setup issues**: Use `reset_foundation_setup_for_testing()` in test fixtures
