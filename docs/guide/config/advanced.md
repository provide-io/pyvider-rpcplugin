...
1. Set proper file permissions (private keys mode 600)
...
1. **Tune message sizes for your data** - Calculate based on largest message
1. **Enable debug logging during development** - Disable in production
1. **Adjust keepalive for network conditions** - Frequent for mobile/WiFi, infrequent for datacenter
1. **Profile before tuning buffers** - Measure, don't guess

### Security

1. **Always use mTLS in production** - Non-negotiable
1. **Proper certificate permissions** - Private keys mode 600
1. **Secure magic cookies** - Use cryptographically secure random strings
1. **Network isolation** - Unix sockets for same-host, firewall rules for TCP

### Rate Limiting

1. **Start conservative** - Begin with lower limits, increase based on monitoring
1. **Monitor metrics** - Track rate limiting events and patterns
1. **Client-side handling** - Always implement exponential backoff
1. **Burst capacity** - Set 2-5x higher than sustained rate
1. **Environment specific** - Different limits for dev/staging/production

### Operations

1. **Implement health checks** - Enable `PLUGIN_HEALTH_SERVICE_ENABLED=true`
1. **Structured logging** - Disable emoji for production log aggregation
1. **Graceful shutdown** - Handle SIGTERM/SIGINT properly
1. **Resource limits** - Set appropriate memory/CPU limits
1. **Process management** - Use systemd or equivalent
