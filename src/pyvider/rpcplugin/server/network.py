...
def _server_cert_alt_names() -> list[str]:
    """The names the auto-generated server certificate must vouch for.

    The handshake line advertises the address the transport bound to, and a TLS
    client dialling an address verifies it against `iPAddress` entries. A
    certificate carrying only `DNS:localhost` therefore cannot be verified
    against `127.0.0.1:<port>` -- the channel never becomes ready, and the
    failure surfaces as a handshake timeout for a handshake that succeeded.

    Loopback is always covered because that is what the TCP transport binds by
    default. A configured host is added so a server told to bind elsewhere
    still presents a certificate for the address its clients will dial.

    `localhost` stays: go-plugin's host verifies against that name
    unconditionally (go-plugin/client.go:690), whatever the certificate says.
    """
    names = ["localhost", "127.0.0.1", "::1"]
    host = getattr(rpcplugin_config, "plugin_server_host", None)
