...
class ServerNetworkMixin:
    ...
    def _load_server_certificate(
        ...
        # PROOF: Log what environment variables we're reading (or NOT reading)
        import os
        tls_key_type_env = os.getenv("TLS_KEY_TYPE")
        tls_curve_env = os.getenv("TLS_CURVE")
        logger.info(f"🔍 PROOF: TLS_KEY_TYPE env var = {tls_key_type_env!r}")
        logger.info(f"🔍 PROOF: TLS_CURVE env var = {tls_curve_env!r}")
        logger.info(f"🔍 PROOF: About to call create_self_signed_server_cert with NO key_type/curve params")
        ...
        try:
            cert_obj = Certificate.create_self_signed_server_cert(
                ...
                validity_days=365,
                alt_names=["localhost"],
                # BUG: Not passing key_type or ecdsa_curve! Always uses defaults!
            )
        except Exception as exc:
        ...
        # PROOF: Log what cert type was actually generated
        logger.info(f"🔍 PROOF: Generated cert with algorithm: {getattr(cert_obj, 'algorithm', 'Unknown')}")
