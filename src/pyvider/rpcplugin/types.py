...
# Abstract base classes that implement the protocols
class BaseHandler(abc.ABC):
    """
    Abstract base class that implements the RPCPluginHandler protocol.
    
    This class provides a common base for handler implementations and ensures
    they conform to the RPCPluginHandler protocol.
    """
...
class BaseProtocol(abc.ABC):
    """
    Abstract base class that implements the RPCPluginProtocol protocol.
    
    This class provides a common base for protocol implementations and ensures
    they conform to the RPCPluginProtocol protocol.
    """
    ...
    def get_method_type(self, method_name: str) -> str:
        ...
        Returns:
            String representing the method type (e.g., "unary_unary", "stream_stream")
        """
        # Default implementation returns "unary_unary" for all methods
        logger.debug(f"🧰🔍⚠️ Using default method type for {method_name}: unary_unary")
        return "unary_unary"


class BaseTransport(abc.ABC):
    """
    Abstract base class that implements the RPCPluginTransport protocol.
    
    This class provides a common base for transport implementations and ensures
    they conform to the RPCPluginTransport protocol.
    """
