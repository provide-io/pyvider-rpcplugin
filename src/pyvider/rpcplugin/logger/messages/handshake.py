...
class HandshakeLogMessages(Enum):
    ...
    handshake_error_validate_transport_failed = "🤝🚂❌ Unsupported transport detected: {transport_name}"
    handshake_debug_validate_transport_success = "🤝🚂✅ Transport '{transport_name}' is supported."

    # Protocol Version Negotiation
    handshake_debug_protocol_negotiation_start = "🤝🔄 Negotiating protocol version. Server supports: {server_versions}"
    ...
    handshake_error_transport_negotiation_no_options = "🗣️🚊❌ (Transport Negotiation: Failed) => No transport options provided"
    handshake_debug_transport_selected_tcp = "🗣️🚊👥 (Transport Negotiation: Selected TCP) => TCP transport is available"
    handshake_debug_transport_selected_unix = "🗣️🚊🧦 (Transport Negotiation: Selected Unix) => Unix socket transport is available"
    handshake_error_transport_negotiation_failed = "🗣️🚊❌ (Transport Negotiation: Failed) => No supported transport found"
    ...
    handshake_debug_validate_handshake_parts = "🔍✅ TypeGuard: Handshake response format validated."
    handshake_error_invalid_handshake_parts = "🔍❌ Invalid handshake response format."

    # Magic Cookie Validation
    handshake_debug_magic_cookie_validation_start = "🍪🔍 Starting magic cookie validation..."
    ...
    handshake_error_magic_cookie_mismatch = "🍪❌ cookie_provided does not match required cookie_value"

    # Handshake Response Building
    handshake_debug_build_handshake_response_start = "🤝📝🔄 Building handshake response..."
    ...
    handshake_debug_unix_transport_listening = "🤝📝🔄 Waiting for Unix transport to listen..."
    handshake_debug_unix_transport_received = "🤝📝✅ Unix transport endpoint received: {endpoint}"
    handshake_error_unsupported_transport_type = "🤝📝❌ Unsupported transport type: {transport_name}"
    handshake_debug_handshake_base_response = "🤝📝🔄 Base response structure: {response_parts}"
    handshake_debug_handshake_cert_processing = "🤝🔐🔄 Processing server certificate..."
    ...
    handshake_debug_handshake_response_success = "🤝📝✅ Handshake response successfully built: {handshake_response}"
    handshake_error_handshake_response_failed = "🤝📝❌ Handshake response build failed: {error}"

    # Handshake Response Parsing
    handshake_debug_handshake_parsing_start = "📡🔍 Starting handshake response parsing for: {response}"
    handshake_debug_handshake_parsing_split = "📡🔍 Split handshake response into parts: {parts}"
    handshake_error_invalid_handshake_format = "📡❌ Invalid handshake response format. Expected 6 parts, got {parts_count}"
    handshake_error_invalid_network_type = "📡❌ Invalid network type: {network}"
    handshake_debug_handshake_cert_padding = "📡🔐 Restored certificate padding for handshake parsing."
