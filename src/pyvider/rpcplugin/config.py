...
from typing import Any, Literal, cast, get_args # For TRANSPORT_TYPES
...
def load_config_from_file(config_file: str | Path) -> None:
    ...
    Raises:
        ConfigError: If the file format is not supported or loading fails
    """
    path = Path(config_file) if isinstance(config_file, str) else config_file

    if not path.exists():
        logger.error(f"⚙️❌ Configuration file not found: {path}")
        raise ConfigError(message=f"Configuration file not found at path: {path}", hint="Verify the file path is correct and the file exists.")
    ...
    try:
        ...
        if suffix == ".env":
            ...
            raise ConfigError(message=f"Unsupported configuration file format: '{suffix}'.", hint="Supported file formats are: .env, .json, .yaml, .yml. Please use one of these extensions.")

        # Reload configuration from environment
        rpcplugin_config.config = get_config()
        logger.debug(f"⚙️📂✅ Successfully loaded configuration from {path}")

    except ConfigError: # Let ConfigErrors from helpers or get_config() pass through
            # Assuming logger call is handled by the raiser or will be caught higher up if needed.
            raise
    except Exception as e: # Wrap other unexpected errors
        logger.error(
            f"⚙️📂❌ Unexpected error loading configuration from {path}", extra={"error": str(e)}
        )
        raise ConfigError(
            message=f"An unexpected error occurred while loading configuration from {path}.",
            hint="Review the file for syntax issues or check application logs for more details."
        ) from e
...
def _load_dotenv_file(path: Path) -> None:
    ...
    try:
        ...
        raise ConfigError(message=f"Error parsing .env file at path: {path}.", hint="Check the file for correct KEY=VALUE formatting. Ensure there are no syntax errors.") from e
...
def _load_json_file(path: Path) -> None:
    ...
    try:
        ...
        raise ConfigError(message=f"Error parsing JSON configuration file at path: {path}.", hint="Check the file for correct JSON syntax (e.g., commas, quotes, brackets).") from e
...
def _load_yaml_file(path: Path) -> None:
    ...
    Raises:
        ConfigError: If loading fails or PyYAML is not installed
    """
    ...
    try:
        try:
            import yaml
        except ImportError as e_import: # Catch ImportError specifically
            logger.error("⚙️📂❌ PyYAML is required for YAML configuration")
            raise ConfigError(
                message="PyYAML package is required to load YAML configuration files.",
                hint="Please install it by running 'pip install PyYAML' or adding 'PyYAML' to your project dependencies."
            ) from e_import

        with open(path, "r", encoding="utf-8") as f:
            config_data = yaml.safe_load(f)

        import json # Ensure json is available for json.dumps
        ...
        # Check if it's already a ConfigError from PyYAML import failure
        if isinstance(e, ConfigError):
            raise
        raise ConfigError(message=f"Error parsing YAML configuration file at path: {path}.", hint="Check the file for correct YAML syntax (e.g., indentation, colons).") from e
...
[end of src/pyvider/rpcplugin/config.py]
