...
class UnixSocketTransport(RPCPluginTransport):
    ...
    async def connect(self, endpoint: str) -> None:
        """Connect as a client to the Unix socket."""
        logger.debug(f"🔌 connect called with endpoint={endpoint}")
        
        # Fix: Properly handle various unix socket path formats 
        if endpoint.startswith("unix:"):
            # Strip unix: prefix (handles unix:/path and unix:///path formats)
    ...
    async def X_2connect(self, endpoint: str) -> None:
        """
