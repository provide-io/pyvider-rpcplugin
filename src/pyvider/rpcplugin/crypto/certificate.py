...
class Certificate:
    ...
    )  # Actual cryptography cert object
    ...
    )  # Public PEM certificate string
    # Keep key PEM out of repr
    key: str | None = field(
        init=False, default=None, repr=False
    )  # Public PEM key string (sensitive)
    ...
    def __attrs_post_init__(self) -> None:
        ...
        try:
            if self.generate_keypair:
                ...
                if (
                    self._cert is None
                ):  # Should not happen if _create_x509_certificate is correct
    ...
    def _create_x509_certificate(
        ...
        try:
            ...
            builder = builder.add_extension(
                ...
                ),  # path_length is None for non-CA or CA with no limit
                critical=True,
            )

            if is_ca:
                # If a self-signed cert is acting as CA AND end-entity, it needs usages for both.
                is_rsa_key = isinstance(self._base.public_key, rsa.RSAPublicKey)
                is_ec_key = isinstance(self._base.public_key, ec.EllipticCurvePublicKey)
                builder = builder.add_extension(
                    x509.KeyUsage(
                        digital_signature=True,  # Needed if also acting as end-entity
                        key_encipherment=is_rsa_key, # Needed for RSA key exchange if also end-entity
                        key_agreement=is_ec_key,    # Needed for ECDH if also end-entity
                ...
                builder = builder.add_extension(
                    x509.KeyUsage(
                        ...
                        else False,  # For server RSA key exchange
                        key_agreement=True
    ...
    def create_signed_certificate(
        ...
        new_cert_obj = cls(
            ...
            validity_days=validity_days,
            alt_names=alt_names
            or [common_name],  # Default SAN to common_name if not provided
            key_type=key_type,
            key_size=key_size,
            ecdsa_curve=ecdsa_curve,
        )
