...
class RPCPluginServer(ABC, Generic[ServerT, HandlerT, TransportT, ProtocolT]):
        ...
        try:
            ...
            if self._transport and self._transport.endpoint:
                if isinstance(self._transport, UnixSocketTransport):
                    if not self._transport.path or not os.path.exists(self._transport.path):
                        raise TransportError(f"Unix socket file {self._transport.path} does not exist.")
            ...
            raise TransportError(f"Server failed to signal readiness within {timeout}s.") from e
    ...
    def _generate_server_credentials(self) -> grpc.ServerCredentials:
        ...
        if server_cert_conf and server_key_conf:
            ...
            raise SecurityError("Server certificate or key not configured for secure mode.")

        if not (self._server_cert_obj and self._server_cert_obj.cert and self._server_cert_obj.key):
            raise SecurityError("Server certificate object is invalid or missing PEM data.")
    ...
    async def _setup_server(self, client_cert: str | None) -> None:
        try:
            interceptors = [RateLimitingInterceptor(self._rate_limiter)] if self._rate_limiter else []
            ...
            creds = self._generate_server_credentials() if rpcplugin_config.auto_mtls_enabled() or rpcplugin_config.get("PLUGIN_SERVER_CERT") else None
            ...
            bind_address = f"unix:{endpoint}" if isinstance(self._transport, UnixSocketTransport) else endpoint
    ...
    async def _negotiate_handshake(self) -> None:
        ...
        if not self.transport:
            self._transport_name, self._transport = await negotiate_transport(self._handshake_config.supported_transports)
        else:
