    ...
    def _generate_server_credentials(self) -> grpc.ServerCredentials:
        ...
        if server_cert_conf and server_key_conf:
            ...
            try:
                ...
                logger.debug("🛎️🔐 Server certificate and key loaded from configuration.")
            except Exception as e:
                logger.error(f"🛎️🔐❌ Failed to load server certificate/key from configuration: {e}", extra={"trace": traceback.format_exc()})
                raise SecurityError(f"Failed to load server certificate/key from configuration: {e}") from e
        elif auto_mtls:
            # PLUGIN_AUTO_MTLS is True, but server certs are NOT provided by user.
            # Implement "automatic" server cert generation.
            logger.info("🛎️🔐 `PLUGIN_AUTO_MTLS` is true and server certs not provided. Auto-generating self-signed server certificate.")
            try:
                ...
                logger.info(f"🛎️🔐 Successfully auto-generated self-signed server certificate and key. Subject: {self._server_cert_obj.subject}")
            except Exception as e:
                logger.error(f"🛎️🔐❌ Failed to auto-generate self-signed server certificate: {e}", extra={"trace": traceback.format_exc()})
                raise SecurityError(f"Failed to auto-generate self-signed server certificate for automatic mTLS: {e}") from e
        else:
            ...
            if server_cert_conf and not server_key_conf:
                ...
                logger.error("🛎️🔐❌ _generate_server_credentials called inappropriately for an insecure configuration (auto_mtls=false, no certs).")
                raise SecurityError("Credential generation called unexpectedly for an insecure configuration.")

        if not self._server_cert_obj or not self._server_cert_obj.cert or not self._server_cert_obj.key:
            raise SecurityError(
        ...
        try:
            ...
            logger.error(f"🛎️🔐❌ Failed to encode server certificate/key to bytes: {e}", extra={"trace": traceback.format_exc()})
        ...
        if auto_mtls:
            if client_root_certs_conf:
                logger.debug("🛎️🔐 mTLS enabled with user-provided `PLUGIN_CLIENT_ROOT_CERTS`. Client certificate validation will be required.")
                require_auth = True
                try:
                    if client_root_certs_conf.startswith("file://"):
                        ...
                        logger.info(f"🛎️🔐 Loaded client root CAs for mTLS from {ca_path}.")
                    else:
                        client_ca_pem_bytes = client_root_certs_conf.encode('utf-8')
                        logger.info("🛎️🔐 Loaded client root CAs for mTLS from configuration string.")
                except Exception as e:
                    logger.error(f"🛎️🔐❌ Failed to load `PLUGIN_CLIENT_ROOT_CERTS`: {e}", extra={"trace": traceback.format_exc()})
                    raise SecurityError(f"Failed to load `PLUGIN_CLIENT_ROOT_CERTS`: {e}") from e
            elif generated_self_signed_server_cert:
                logger.info("🛎️🔐 Server certificate auto-generated for `PLUGIN_AUTO_MTLS`, and no `PLUGIN_CLIENT_ROOT_CERTS` provided. Operating with server-side TLS (client certs not required/validated by server).")
                require_auth = False
            else: # auto_mtls is true, server certs were user-provided, but no client_root_certs_conf.
                logger.info("🛎️🔐 `PLUGIN_AUTO_MTLS` is true, server certs provided by user, but no `PLUGIN_CLIENT_ROOT_CERTS`. Operating with server-side TLS (client certs not required/validated by server).")
                require_auth = False
        elif rpcplugin_config.get("PLUGIN_SERVER_CERT"): # auto_mtls is FALSE, but server cert is provided
            logger.info("🛎️🔐 Server-only TLS configured (PLUGIN_AUTO_MTLS is false, server cert provided). Client certificates will not be required by server.")
            require_auth = False
        ...
        try:
            ...
            if require_auth:
                logger.debug("🛎️🔐✅ mTLS server credentials created successfully (client auth required).")
            else:
                logger.debug("🛎️🔐✅ Server-only TLS credentials created successfully (client auth not required).")
            return creds
        except Exception as e:
            logger.error(f"🛎️🔐❌ Failed to create grpc.ssl_server_credentials: {e}", extra={"trace": traceback.format_exc()})
            raise SecurityError(f"Failed to create gRPC SSL server credentials: {e}") from e
