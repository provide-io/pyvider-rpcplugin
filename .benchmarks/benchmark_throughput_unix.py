...
from pyvider.rpcplugin import plugin_protocol, plugin_server  # Import plugin_protocol
...
async def run_client_direct_transport(socket_path, num_requests):
    ...
    try:
        ...
        await asyncio.wait_for(
            channel.channel_ready(), timeout=2.0
        ...
        for i in range(num_requests):
            ...
            try:
                response = await stub.Echo(
                    request, timeout=0.5
                )  # Short timeout for individual req
                if not response or not response.message:
                    logger.warning(f"Received empty/invalid response for request {i}")
            except grpc.aio.AioRpcError as e:
                logger.error(
                    f"RPC error on request {i}: {e.details()} (code: {e.code()})"
                )
                if e.code() in (
                    ...
                    logger.error(
                        "Server seems unavailable or request timed out. Stopping client benchmark."
                    )
                    break
        ...
        if total_time > 0:
            ...
            rps = float(
                "inf"
            )  # Effectively infinite if time was zero for some requests
...
async def main_benchmark():
    ...
    server_task = asyncio.create_task(
        run_server(socket_path, server_ready_event, server_stop_event)
    )
    ...
    if (
        not os.path.exists(socket_path) and not server_task.done()
    ):  # Check if server task is already done (e.g. crashed)
        logger.error(
            f"Server did not create socket file at {socket_path} and task is not done. Aborting client."
        )
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Server did not stop gracefully in time. Forcibly cancelling task."
            )
            ...
            logger.info(
                "Server task was already cancelled (e.g. from inner exception)."
            )
        ...
        if os.path.exists(socket_path):
            ...
            try:
                ...
                logger.warning(
                    f"Error removing socket file {socket_path} during cleanup: {e}"
                )

    target_rps = 50000
    if actual_rps >= target_rps:
        logger.info(
            f"SUCCESS: Achieved {actual_rps:.2f} RPS (Target: {target_rps}+ RPS)"
        )
    else:
        logger.warning(
            f"PERFORMANCE NOTE: Achieved {actual_rps:.2f} RPS (Target: {target_rps}+ RPS)."
        )
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Starting Unix Domain Socket Throughput Benchmark with log level {log_level_env} (Note: effective level depends on Pyvider global config)..."
    )
